...
double cal_fps(fpsHandle* handle, const char* name, bool print = false) {
  ...
  double elapsed = (handle->last.tv_sec - handle->start.tv_sec) + (handle->last.tv_nsec - handle->start.tv_nsec) / 1e9;
...
void collect_children(pid_t pid, std::vector<pid_t>& all) {
  std::string   task_path = "/proc/" + std::to_string(pid) + "/task/" + std::to_string(pid) + "/children";
    ...
    if (command == "set") {
      ...
      try {
        ...
        if (param_name == "save_map" && value == 1) {
          RCLCPP_INFO(rclcpp::get_logger("command_processor"), "Successfully set %s = %d", param_name.c_str(), value);
          {
            ...
            RCLCPP_INFO(rclcpp::get_logger("command_processor"), "Map save triggered, will transfer to [%s/%s]", map_dir.c_str(), map_name.c_str());
        ...
        if (result == 0) {
          ...
          RCLCPP_INFO(rclcpp::get_logger("command_processor"), "Device is busy transferring file, please wait for current transfer to complete");
        } else {
          RCLCPP_ERROR(rclcpp::get_logger("command_processor"), "Failed to set %s = %d, error: %d", param_name.c_str(), value, result);
        }
      }
...
static bool convert_calib_to_cam_in_ex(const std::string& calib_path, const std::filesystem::path& out_path) {
  try {
    ...
    auto       get_i = [&](const char* key, int def) -> int { return (cam0 && cam0[key]) ? cam0[key].as<int>() : def; };
    auto       get_d = [&](const char* key, double def) -> double { return (cam0 && cam0[key]) ? cam0[key].as<double>() : def; };
    ...
    case LIDAR_DT_DEV_STATUS:
      ...
      if (g_show_fps) {
        ...
        printf("\n [dev_info] [dtof]: configured_odr: %.2f HZ, tx_odr: %.2f HZ, rx_odr: %.2f HZ \n", ((float)dev_info_data->dtof_sensor.configured_odr) / 1000,
               ((float)dev_info_data->dtof_sensor.tx_odr) / 1000, cal_fps(&dtof_rx_fps, "dtof_rx"));
...
static void lidar_device_callback(const lidar_device_info_t* device, bool attach) {
  ...
  if (attach == true) {
    ...
    if (!isUsb3OrHigher(TARGET_VENDOR, TARGET_PRODUCT)) {
      RCLCPP_FATAL(rclcpp::get_logger("device_cb"), "Device connected to USB 2.0 port. This device requires USB 3.0 or higher. Exiting program.");
    ...
    std::filesystem::path per_con_log_root_dir;
    {
      ...
      std::filesystem::path base_log_dir = log_root_dir_.empty() ? std::filesystem::path(config_dir) : log_root_dir_;
      ...
      if (per_con_dir_err) {
        RCLCPP_WARN(rclcpp::get_logger("device_cb"), "Failed to create per-connection log directory %s: %s", per_con_log_root_dir.c_str(),
    ...
    if (elapsed.count() >= 60) {
      RCLCPP_FATAL(rclcpp::get_logger("device_cb"), "Software connection timed out after 60 seconds. Exiting program.");
    ...
    if (lidar_get_version(odinDevice, &version)) {
      ...
      RCLCPP_INFO(rclcpp::get_logger(__func__), "ros_driver_version:%s, recommended_firmware_version:%d.%d.%d", ros_driver_version,
      ...
      RCLCPP_INFO(rclcpp::get_logger(__func__), "kernel_version: V%d.%d.%d", version.kernel_version.major, version.kernel_version.minor,
      ...
      RCLCPP_INFO(rclcpp::get_logger(__func__), "Daemon_proc_version: V%d.%d.%d", version.Daemon_proc_version.major, version.Daemon_proc_version.minor,
      ...
      const bool soc_version_too_low =
          ...
          (version.soc_version.major == required_firmware_version_major && version.soc_version.minor < required_firmware_version_minor) ||
          (version.soc_version.major == required_firmware_version_major && version.soc_version.minor == required_firmware_version_minor &&
    ...
    if (g_ros_object) {
      char fw_buf[48];
      char algo_buf[48];
      std::snprintf(fw_buf, sizeof(fw_buf), "%d.%d.%d", version.soc_version.major, version.soc_version.minor, version.soc_version.patch);
      std::snprintf(algo_buf, sizeof(algo_buf), "%d.%d.%d", version.slam_version.major, version.slam_version.minor, version.slam_version.patch);
      std::string sn;
    ...
    if (g_dtof_fps == 100) {
      ...
      dtofpara.odr = LIDAR_DEPTH_ODR_14_5HZ;
      RCLCPP_WARN(rclcpp::get_logger("ros[host_sdk_sample]"), "Invalid dtof_fps value: %d, using default 14.5fps", g_dtof_fps);
    }
    ...
    RCLCPP_INFO(rclcpp::get_logger("ros[host_sdk_sample]"), "DTOF sensor frame rate set to %.1f fps", g_dtof_fps / 10.0);
    ...
    if (g_custom_map_mode == 1) {
      ...
      if (result == 0) {
        RCLCPP_INFO(rclcpp::get_logger("command_processor"), "Successfully initialized %s = %d", "save_map", save_map_init_value);
      } else {
        RCLCPP_ERROR(rclcpp::get_logger("command_processor"), "Failed to initialize %s = %d, error: %d", "save_map", save_map_init_value, result);
      }
    ...
    // 根据配置激活或关闭流类型
    RCLCPP_INFO(rclcpp::get_logger("device_cb"), "Stream config: RGB=%d, IMU=%d, ODOM=%d, DTOF=%d, CLOUD_SLAM=%d", g_sendrgb, g_sendimu, g_sendodom, g_senddtof,
                g_sendcloudslam);
    ...
    RCLCPP_INFO(rclcpp::get_logger("device_cb"), "Command interface ready. Use: echo 'set save_map 1' > %s", g_command_file_path.c_str());
...
int main(int argc, char* argv[]) {
  ...
  auto node    = std::make_shared<rclcpp::Node>("odin1");
  g_ros_object = std::make_shared<MultiSensorPublisher>(node);

  mrs_lib::ParamLoader param_loader(node, "odin1");
  ...
  auto srv_set_ae = node->create_service<odin_ros_driver::srv::SetAe>(
      "~/set_ae", [](const std::shared_ptr<odin_ros_driver::srv::SetAe::Request> req, std::shared_ptr<odin_ros_driver::srv::SetAe::Response> res) {
        ...
        lidar_cam_mode_e mode = (req->mode == 1) ? LIDAR_CAM_MODE_MANUAL : LIDAR_CAM_MODE_AUTO;
        int              rc   = lidar_set_ae_param(odinDevice, mode, req->exposure_time, req->gain);
        res->rc               = rc;
        res->success          = (rc == 0);
      });
  ...
  try {
    ...
    g_parser = std::make_shared<odin_ros_driver::YamlParser>(config_path);
